...
!
    ...
    PROGRAM raytracef
      USE mod_raytracef
      IMPLICIT NONE
      INTEGER, PARAMETER :: size = 4000, tilesize = 200, nbuf = 2
      INTEGER :: xtiles, ytiles, xc, yc, ibuf
      INTEGER (KIND=is), ASYNCHRONOUS :: picture(size, nbuf*tilesize)


      xtiles = size/tilesize
      ytiles = size/tilesize
      WRITE (*, *) 'Starting loop processing with asynchronous I/O ... '
      OPEN (20, FILE='result.pnm', ACCESS='STREAM', FORM='UNFORMATTED', &
        STATUS='REPLACE', ASYNCHRONOUS='YES')
      ...
      DO yc = 0, ytiles
        IF (yc>0) THEN
          IF (ibuf==0) WAIT (20)
          WRITE (UNIT=20, ASYNCHRONOUS='YES') picture(:, &
            ibuf*tilesize+1:(ibuf+1)*tilesize)
        END IF
        IF (yc<ytiles) THEN
          ibuf = ibuf + 1
          IF (ibuf==nbuf) THEN
          ...
          DO xc = 0, xtiles - 1
            CALL calc_tile(size, xc*tilesize, yc*tilesize, tilesize, &
              picture(xc*tilesize+1,ibuf*tilesize+1))
          END DO
        END IF
      END DO
      CLOSE (20) ! implies last needed wait
      WRITE (*, *) '... done.'
