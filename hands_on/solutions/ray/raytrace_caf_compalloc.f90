...
!
    ...
    PROGRAM raytracef
      ...
      INTEGER :: xtiles, ytiles, i, xc, yc
      INTEGER :: me, tasks
      TYPE :: pct
        INTEGER (KIND=is), ALLOCATABLE :: data(:, :)
      END TYPE
      TYPE (pct) :: picture[ * ]
      INTEGER (KIND=is) :: tile(tilesize, tilesize)
      DOUBLE PRECISION :: ti
...
!     sync images (1)
      ...
      DO i = me, xtiles*ytiles, tasks
        ...
        CALL calc_tile(size, xc*tilesize, yc*tilesize, tilesize, tile, &
          tilesize)
        picture[ 1 ]%data(xc*tilesize+1:(xc+1)*tilesize, &
          yc*tilesize+1:(yc+1)*tilesize) = tile
      END DO
      ...
      IF (me==1) THEN
        ...
        WRITE (20) picture%data
        CLOSE (20)
        WRITE (*, *) '... done.'
        ti = dwalltime() - ti
        WRITE (*, *) 'I/O rate: ', 4*size**2/1024**2/ti, ' MB/s.'
        DEALLOCATE (picture%data)
      END IF
