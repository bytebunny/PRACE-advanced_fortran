...
!
    ...
    PROGRAM raytracef
      ...
      INTEGER :: xtiles, ytiles, i, ic, it, xc, yc, maxcount
      INTEGER :: count[ * ]
      INTEGER :: me, tasks
      ...
      maxcount = xtiles*ytiles/tasks + 1
      ALLOCATE (tile(tilesize,tilesize,maxcount)[*] )

      count = 0
      DO i = me, xtiles*ytiles, tasks
        xc = mod(i-1, xtiles)
        yc = (i-1)/xtiles
        count = count + 1
        CALL calc_tile(size, xc*tilesize, yc*tilesize, tilesize, &
          tile(1,1,count), tilesize)
      END DO
...
!  collect data on master
        ...
        DO it = 1, tasks
          DO ic = 1, count[ it ]
            ...
            xc = mod(i-1, xtiles)
            yc = (i-1)/xtiles
            picture(xc*tilesize+1:(xc+1)*tilesize, yc*tilesize+1:(yc+1)* &
              tilesize) = tile(:, :, ic)[ it ]
          END DO
        END DO
        ti = dwalltime() - ti
        WRITE (*, *) 'Transfer rate: ', 4*size**2/1024**2/ti, ' MB/s.'
        ...
        WRITE (*, *) '... done.'
        ti = dwalltime() - ti
        WRITE (*, *) 'I/O rate: ', 4*size**2/1024**2/ti, ' MB/s.'
        DEALLOCATE (picture)
      END IF
      DEALLOCATE (tile)
    CONTAINS
